...
def load_local_dataset(
    ...
    """
    Load a preprocessed local dataset to a Huggingface dataset
    Args:
        preprocessed_datadir: directory of the preprocessed dataset
        text_column_name: name of the text transcription column
        sampling_rate: sampling rate of the audio
        use_pseudo_labels: use pseudo label generated by the teacher model or not
    """
    # Get all dataset in .arrow files
    arrow_files = []
    for root, dirs, files in os.walk(preprocessed_datadir):
        for file in sorted(files):
            if file.startswith('data') and file.endswith('.arrow'):
                p = os.path.join(root, file)
                arrow_files.append(os.path.abspath(p))
...
def main():
    ...
    for epoch in range(epochs_trained, num_epochs):
        ...
        train_dataloader = DataLoader(
            ...
            batch_sampler=BucketByAudioLength(dataset=vectorized_datasets["train"])
        )
            ...
            if accelerator.sync_gradients:
                ...
                if training_args.do_eval and (cur_step % eval_steps == 0 or cur_step == total_train_steps):
                    ...
                    validation_dataloader = DataLoader(
                        ...
                        num_workers=dataloader_num_workers,
                        pin_memory=training_args.dataloader_pin_memory,
                        batch_sampler=BucketByAudioLength(dataset=vectorized_datasets["eval"])
                    )
